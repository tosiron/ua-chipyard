...
This guide provides simple hands-on tutorials for Chipyard once it's installed. Make sure you've completed one of the installation guides first: [Docker](INSTALL_DOCKER.md), [Native Linux](INSTALL_LINUX.md), or [Virtual Machine](INSTALL_VM.md).
...
- [ROCC Accelerator Integration](#rocc-accelerator-integration)
...
If you want to implement a ROCC accelerator and couple it with your CPU, use the template code located in [`ROCC.scala`](ROCC.scala), at the root of this repository.
...
1. Copy `ROCC.scala` from the repository root into the following folder:
   ```
...
Create a new Scala file in `chipyard/generators/chipyard/src/main/scala/config` and follow the reference code in [`ROCC.scala`](ROCC.scala) at the repository root.
