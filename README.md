...
- Create a folder under `C:\Users\<username>` with a name of your preference. This tutorial will use `ECEx62` as the folder name.
- Copy the `Dockerfile` to `C:\Users\<username>\ECEx62`.
- Open PowerShell and run the following commands:

```powershell
cd C:\Users\<username>\ECEx62  # Change directory to ECEx62 folder,
                                # where Dockerfile is located
docker build -t ecex62:chipyard .  # Build docker
```
...
- Create a folder named `ECEx62` under your home directory and copy the `Dockerfile` to the folder:
...
```bash
docker build -t ecex62:chipyard .
```
...
```bash
docker run --privileged -it --name ecex62_chipyard ecex62:chipyard /bin/bash
```
